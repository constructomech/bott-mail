...
class Syncer:
    ...
    def _ingest(self, folder: str, fetched) -> tuple[int, int, int, int]:
        """Parse + upsert a batch. Returns (seen, inserted, updated, max_uid)."""
        parsed = []
        max_uid = 0
        for fm in fetched:
        ...
        return result.seen, result.inserted, result.updated, max_uid
    ...
    def sync(self, days: int, folders: list[str]) -> dict:
        ...
        for folder in self._target_folders(folders):
            ...
            s, i, u, max_uid = self._ingest(folder, res.messages)
            seen += s
            inserted += i
            updated += u
